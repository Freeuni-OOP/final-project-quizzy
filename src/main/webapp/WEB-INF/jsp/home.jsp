...
<%
  ...
  List<InboxItem> messagePreviews = (List<InboxItem>) request.getAttribute("messagePreviews");
...
<main class="container home-grid">
  ...
  <aside class="home-sidebar">
    ...
    <section class="panel">
      ...
      <% if (messagePreviews != null && !messagePreviews.isEmpty()) { %>
      <ul class="activity-list">
        <% for (InboxItem item : messagePreviews) { %>
        <li class="activity-item">
          <span><%= WebEscape.html(item.getSenderName()) %>: <%= WebEscape.html(item.getMessage().getBody()) %></span>
        </li>
        <% } %>
      </ul>
      <% } else { %>
